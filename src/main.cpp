...
int tournament(Player& player, Opponent& opponent)
{
    int gamesDraw{ 0 };
    defend::fatigue = 0;
    std::vector<std::string> namesTeamOpponent{ "Колледж Иллинойс", "Колледж Вашингтона", "Колледж Аляски", "Колледж Огайо", "Далласский колледж", "Колледж Техаса", "Колледж Миннесоты", "Колледж Аризоны"};
...
void gameRulesRecord()
{
    ...
    menu::rulesShot =
        ...
        "На попадание влияет:\n1. Защита.\n2. Командный дух.\n\n"
        "Вероятность попадания при лэй-апе или комбинации выше, чем при дальнем и среднем броске,\nно во время их выполнения обороняющаяся команда может сделать перехват или блок-шот."
    ...
    menu::rulesDefense =
        "Немаловажный момент игры - защита. Выбирай её с умом, чтобы реализовать все свои задумки и тактики.\nВнимательно следи за тем, какую защиту использует противник!\n\n"
        "Чтобы изменить защиту во время матча, просто введи 0 в качестве следующего броска.\n\n"
        "Выбирай схему следующим образом:\n"
        "1. Прессинг - эффективная защита (шанс всех бросков снижен на 10%), но будь бдителен, ведь после двух защит в прессинге команда устаёт\n"
        "и следующие две атаки соперника, команда будет в состоянии \"Нет защиты\", чтобы восстановить силы!\n"
        "2. Личная опека - отличная защита от средних и ближних бросков (шанс удачного среднего броска и лэй-апа -20%),\n"
        ...
        "но открывается огромный простор для лэй-апов и средних бросков (шанс удачного лэй-апа и средних бросков + 10%);\n"
...
void situationOne(int& teamSpirit)
{
    ...
    situation.scene =
        ...
        "Вдруг сзади раздался оклик, вы обернулись и увидели Кэти Гринхаус.\n"
        ...
        "— Послушай, — обратилась к тебе Кэти, впрочем, одарив и Билли лёгкой ответной улыбкой,\n"
        "— У вас же завтра игра, верно?\n— Да, — отозвался ты.\n— Отлично! — обрадовалась девушка. "
        ...
        "Кэти страстно увлекалась чирлидингом и мечтала когда-нибудь выиграть чемпионат мира по этому виду спорта,\n"
        "так что не упускала возможности попрактиковаться в выступлениях, и, надо сказать, показывала неплохие результаты.\n"
        "Проблема была только в одном, в феноменальной нестабильности их представлений.\n"
...
void situationTwo(int& teamSpirit)
{
    ...
    if (situation.playerChoice == situation.ACTION && situation.probalityVictory >= history::PROBALITY_WIN)
    {
        situation.riscVictory =
            ...
            "У вас оказалось сотня общих тем для разговора, ты пригласил её на завтрашнюю игру, она обещала прийти,\n"
